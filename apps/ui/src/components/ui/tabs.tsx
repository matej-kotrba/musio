...
export const TabsList = <T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.List
      class={cn(
        "relative flex w-full rounded-lg bg-secondary p-1 text-muted-foreground data-[orientation=vertical]:flex-col data-[orientation=horizontal]:items-center data-[orientation=vertical]:items-stretch",
        local.class
      )}
      {...rest}
    />
...
export const TabsTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <TabsPrimitive.Trigger
      class={cn(
        "peer relative z-10 inline-flex h-7 w-full items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium outline-none transition-colors disabled:pointer-events-none disabled:opacity-50 data-[selected]:bg-zinc-600 data-[selected]:text-foreground",
        local.class
      )}
      {...rest}
    />
